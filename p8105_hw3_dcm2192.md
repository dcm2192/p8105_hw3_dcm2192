...
higher overall total activity than males of the same age. Males older
than age 40 with less than a high school education generally have higher
...
![](p8105_hw3_dcm2192_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->

Based on these plots, both males and females with an education level of
more than high school showed the highest minutes of activity compared to
equivalent or below high school level, and it also showed the notable
outliers among both males and females. Among all education levels and
regardless of sex, there is a sharp decline in activity values toward
the middle of the day, which could be an indication of when the
participant is resting and/or sleeping.
...
The combined dataset `citibike_total`, which contains all four datasets
from January 2020 to July 2024, is comprised of 99485 observations of 9
variables. Its variables include `ride_id`, the type of bike as
`rideable_type`, the day of the week as `weekdays`, the ride time as
`duration`, the names of the start and end stations as
`start_station_name` and `end_station_name`, the designation of either a
Citi Bike member or casual rider as `member_casual`, and the `month` and
...
```
    ...
    ## # A tibble: 8 × 4
    ## # Groups:   member_casual, month [4]
    ##   member_casual month   year  n_obs
    ##   <chr>         <chr>   <chr> <int>
    ## 1 casual        January 2020    984
    ## 2 casual        January 2024   2108
    ## 3 casual        July    2020   5637
    ## 4 casual        July    2024  10894
    ## 5 member        January 2020  11436
    ## 6 member        January 2024  16753
    ## 7 member        July    2020  15411
    ## 8 member        July    2024  36262

January 2020 had the lowest number of causal riders, and July 2024 saw
the highest number of causal riders. Among Citi Bike members, January
2020 had the lowest number of riders, and July 2024 had the highest
number of riders. Regardless of year or membership status, there were
fewer total riders in January than in July. There were also fewer total
riders in January than in July, regardless of year or membership status.
...
![](p8105_hw3_dcm2192_files/figure-gfm/unnamed-chunk-17-1.png)<!-- -->

Based on these plots, the shortest median ride durations for each day
occurred in January of 2024, and the longest median ride durations for
each day occurred in July of 2020. In general, Saturdays and Sundays had
longer median ride durations than weekdays; the only exception was
January 2024 showing that Sundays had among the shortest median ride
durations out of the other days of the week. Additionally, besides
January of 2020, the median ride duration generally increased as the
week progressed toward the weekend.

Plot impact of month, membership, bike type on ride duration
...
citibike_total |> 
  ...
  summarize(median_ride_duration = median(duration)) |> 
  ggplot(aes(x = month, y = median_ride_duration, color = member_casual)) + 
  geom_point() + facet_grid(. ~ rideable_type)
```

    ## `summarise()` has grouped output by 'month', 'member_casual'. You can override
    ## using the `.groups` argument.

![](p8105_hw3_dcm2192_files/figure-gfm/unnamed-chunk-18-1.png)<!-- -->

In 2024, median ride duration among members on both classic and electric
bikes were shorter than median ride duration among casual riders on both
bike types. Among members in January, median ride duration on classic
bikes were similar to median ride duration on electric bikes in the same
month, but among casual riders in January, classic bikes had a longer
median ride duration than electric bike duration. In July, members rode
classic bikes for a shorter duration than members who rode electric
bikes. However, in July, casual riders rode classic bikes for a longer
duration than casual riders of electric bikes.
